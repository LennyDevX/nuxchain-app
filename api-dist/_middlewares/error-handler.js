...
export function errorHandler(error, res) {
    ...
    if (!(error instanceof ApiError)) {
        ...
        normalizedError = new ApiError(error.message || 'Something went wrong', statusCode, errorType, process.env.NODE_ENV === 'development' ? { stack: error.stack } : {});
    }
    else {
        normalizedError = error;
    }
    // Loguear error
    logError(`API Error [${normalizedError.errorType}]`, normalizedError, { statusCode: normalizedError.statusCode });
    // Preparar respuesta
    const errorResponse = {
