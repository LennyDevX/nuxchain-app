        ...
        for (const token of tokens) {
            if (SYNONYMS[token]) {
                SYNONYMS[token].forEach(syn => {
                    ...
                    if (synNorm && synNorm.length > 2)
                        expanded.add(synNorm);
                });
            }
        }
...
function applySemanticBoost(doc, query, baseScore) {
    ...
    const matchedDomainTerms = domainTerms.filter(term => docLower.includes(term) && queryTerms.some(qt => qt.includes(term) || term.includes(qt)));
...
async function batchEmbedMultipleTexts(texts) {
    ...
    try {
        const ai = new GoogleGenAI({ apiKey });
        const truncatedTexts = texts.map(text => text.length > 8000 ? text.substring(0, 8000) : text);
        const response = await ai.models.embedContent({
            model: 'gemini-embedding-001',
