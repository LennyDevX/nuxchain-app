...
class TokenCountingService {
    ...
    async optimizeContextLength(context, maxTokens = 4000, model = DEFAULT_MODEL) {
        ...
        try {
            ...
            const lastSentenceEnd = Math.max(truncatedContext.lastIndexOf('.'), truncatedContext.lastIndexOf('!'), truncatedContext.lastIndexOf('?'), truncatedContext.lastIndexOf('\n'));
    ...
    getStats() {
        return {
            ...this.tokenStats,
            estimatedTotalCost: this.estimateCost(this.tokenStats.totalInputTokens, this.tokenStats.totalOutputTokens, DEFAULT_MODEL, this.tokenStats.totalCachedTokens)
        };
    }
    /**
    ...
    updateStatsFromResponse(usageMetadata) {
        ...
        if (promptTokenCount)
            this.tokenStats.totalInputTokens += promptTokenCount;
        if (candidatesTokenCount)
            this.tokenStats.totalOutputTokens += candidatesTokenCount;
