...
function detectBlockchainQuery(message) {
    ...
    const isUserNFTQuery = (/\b(mis?|my|cuántos?|cuantos?|tengo|have|minteado|minted|listado|listed)\b/.test(text) &&
        text.includes('nft')) || /\b(mis listings|my listings|nfts listados|nfts for sale|mis nfts?|my nfts?)\b/.test(text);
    if (isUserNFTQuery) {
...
async function streamHandler(req, res) {
    ...
    try {
        ...
        const walletAddress = (headers['x-wallet-address'] || '').trim();
            ...
            try {
                ...
                if (!subData) {
                    ...
                    if (doc.exists) {
                        subData = doc.data();
                        try {
                            await kv.set(cacheKey, subData, { ex: 300 });
                        }
                        catch { /* ok */ }
                    }
                }
                if (subData && subData.status === 'active') {
                    ...
                    else if (expiryRaw && '_seconds' in expiryRaw) {
        ...
        if (subscriptionTier === 'free') {
            ...
            try {
                ...
                if (count === 1)
                    await kv.expire(dailyKey, 86400); // 24h TTL
