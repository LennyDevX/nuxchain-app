 ...
 */
async function getUniswapPrice(tokenAddress, chainId, decimals, apiKey) {
    try {
        ...
        if (!classicQuote)
            return null;
        const outputAmount = classicQuote.outputAmount;
        if (!outputAmount)
            return null;
...
async function getCoinGeckoPrices() {
    ...
    const response = await fetch(`${COINGECKO_API_BASE}/simple/price?ids=${ids}&vs_currencies=usd&include_24hr_change=true&include_24hr_vol=true`, {
 ...
 */
async function getUniswapPrices(apiKey) {
    ...
    const results = await Promise.all(TRACKED_TOKENS.map(async (token) => {
        const cgData = cgPrices?.find(p => p.id === token.id);
        const uniswapPrice = await getUniswapPrice(token.address, token.chainId, token.decimals, apiKey);
        return {
            id: token.id,
            symbol: token.symbol,
            name: token.name,
            ...
            color: token.color,
            source: uniswapPrice !== null ? 'uniswap' : 'coingecko',
        };
    }));
    return results;
}
export default async function handler(req, res) {
    ...
    try {
        ...
        const fallback = TRACKED_TOKENS.map(t => ({
            id: t.id,
            symbol: t.symbol,
            name: t.name,
            price: 0,
