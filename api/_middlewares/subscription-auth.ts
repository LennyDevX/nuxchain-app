...
import type { SkillId } from '../../src/constants/subscription.js';
...
export async function checkSkillAccess(
  ...
  try {
    subData = await kv.get<Record<string, unknown>>(cacheKey);
  } catch { /* fail open */ }
  ...
  if (!subData) {
    ...
    try {
      ...
      // Cache it
      try { await kv.set(cacheKey, subData, { ex: KV_TTL }); } catch { /* ok */ }
    } catch (err) {
  ...
  // ── Check active ─────────────────────────────────────────────────────────
  const expiryRaw = subData.expiryDate as { _seconds?: number; toDate?: () => Date } | string | Date | undefined;
  let expiry: Date;
  if (expiryRaw instanceof Date) {
    ...
    expiry = new Date((expiryRaw._seconds as number) * 1000);
  } else {
    expiry = new Date(0);
  }
  ...
  // ── Check skill access ───────────────────────────────────────────────────
  const activeSkills = (subData.activeSkills as SkillId[]) || [];
  const addOns = (subData.addOns as SkillId[]) || [];
  ...
  return {
    tier: subData.tier as 'pro' | 'premium',
    activeSkills,
    addOns,
    expiryDate: expiry,
  };
 ...
 */
export async function skillsRateLimit(
  req: Request,
  res: Response,
  wallet: string,
  tier: 'pro' | 'premium'
