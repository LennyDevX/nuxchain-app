...
class EmbeddingsCacheService {
   ...
   */
  async getEmbedding(text: string, category?: string): Promise<number[]> {
    const cacheKey = `emb:${Buffer.from(text).toString('base64').substring(0, 100)}`;
    
    // Try to get from cache
    const cached = await kvCache.get<EmbeddingData>(cacheKey, {
   ...
   */
  async batchGenerateEmbeddings(queries: Array<{ text: string; category: string }>): Promise<void> {
    ...
    for (let i = 0; i < queries.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(({ text, category }) => 
          this.getEmbedding(text, category).catch(err => {
   ...
   */
  async precomputeKnowledgeBase(): Promise<void> {
   ...
   */
  async searchSimilar(): Promise<EmbeddingData[]> {
