...
import type { UserBlockchainData } from '../types/index.js';
...
function formatPOL(wei: string | null | undefined): string {
  if (!wei || wei === '0') return '0 POL';
  const bn = BigInt(wei);
  const pol = Number(bn) / 1e18;
  return `${pol.toFixed(4)} POL`;
}

function sumWei(items: Array<{ amount: string }> | undefined): string {
  if (!items || items.length === 0) return '0 POL';
...
function parseGraphResponse(data: any, wallet: string): UserBlockchainData {
  ...
  const activeListings = (data.activeListings || []).map((l: Record<string, string>) => ({
  ...
  const recentNFTs = (data.recentMints || []).map((n: Record<string, string>) => ({
  ...
  const recentDeposits = (data.recentDeposits || []).map((d: Record<string, string>) => ({
    amount: formatPOL(d.amount),
    lockupDuration: Number(d.lockupDuration),
    timestamp: Number(d.timestamp),
  }));

  const recentActivities = (data.recentActivities || []).map((a: Record<string, string>) => ({
    type: a.type || '',
 ...
 */
export async function fetchUserBlockchainData(wallet: string): Promise<UserBlockchainData | null> {
 ...
 */
export function formatUserContextForAI(data: UserBlockchainData): string {
  const lines: string[] = [
  ...
  lines.push(
    ``,
    `-- NFTs (Plataforma Nuxchain) --`,
    `  Minteados por ti: ${data.nftsMintedCount}`,
    `  Vendidos: ${data.nftsSoldCount}`,
    `  Comprados: ${data.nftsBoughtCount}`,
    `  Royalties ganadas (últimos 7 días): ${data.totalRoyaltiesEarned}`,
  );
  ...
  lines.push(
    ``,
    `-- MARKETPLACE --`,
    `  Nivel marketplace: ${data.marketplaceLevel} | XP: ${data.marketplaceXP}`,
