/**
 * 🎯 Token Counting Service for Gemini API (Vercel Serverless)
 * 
 * Features:
 ...
 * Adapted for Vercel serverless environment (stateless)
 */
...
interface MultiPartTokenResult {
  totalTokens: number;
  systemTokens: number;
...
class TokenCountingService {
  ...
  constructor() {
    ...
    console.log('✅ Token Counting Service initialized (Vercel mode)');
  }
   ...
   */
  async countTokens(content: string, model: string = DEFAULT_MODEL): Promise<TokenCountResult> {
    try {
      ...
      return {
        totalTokens,
        estimatedCost: this.estimateCost(totalTokens, 0, model).totalCost,
        charCount: content.length,
        tokensPerChar: totalTokens / content.length
      };
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.warn('⚠️ Token counting failed, using estimation:', message);
   ...
   */
  async countMultiPartTokens(parts: ContentParts, model: string = DEFAULT_MODEL): Promise<MultiPartTokenResult> {
    try {
      const { systemInstruction, context, message, history = [] } = parts;

      // Build contents array for counting
      const contentsToCount: Array<{ role: string; parts: Array<{ text: string }> }> = [];
      ...
      for (const msg of history) {
        ...
        if (text) {
          contentsToCount.push({ role: 'user', parts: [{ text }] });
        }
      }
      ...
      const breakdown: MultiPartTokenResult = {
        totalTokens: result.totalTokens || 0,
        systemTokens: systemInstruction ? this.quickEstimate(systemInstruction) : 0,
        contextTokens: context ? this.quickEstimate(context) : 0,
        messageTokens: message ? this.quickEstimate(message) : 0,
        historyTokens: 0
      };
      ...
      for (const msg of history) {
        const text = msg.parts?.[0]?.text || '';
        breakdown.historyTokens += this.quickEstimate(text);
      }
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.warn('⚠️ Multi-part token counting failed:', message);
   ...
   */
  quickEstimate(text: string, model: string = DEFAULT_MODEL): number {
    if (!text) return 0;
   ...
   */
  estimateTokens(content: string, model: string = DEFAULT_MODEL): TokenCountResult {
   ...
   */
  estimateMultiPartTokens(parts: ContentParts, model: string = DEFAULT_MODEL): MultiPartTokenResult {
    const { systemInstruction = '', context = '', message = '', history = [] } = parts;
    const limits = this.modelLimits[model] || this.modelLimits['gemini-3.1-flash-lite'];
...
const prices = pricing[model] || pricing['gemini-3.1-flash-lite'];
   ...
   */
  isCacheWorthy(content: string, model: string = DEFAULT_MODEL): CacheWorthiness {
const limits = this.modelLimits[model] || this.modelLimits['gemini-3.1-flash-lite'];
   ...
   */
  getStats(): TokenStats & { estimatedTotalCost: CostEstimate } {
    return {
      ...this.tokenStats,
   ...
   */
  updateStatsFromResponse(usageMetadata: UsageMetadata): void {
...
// Export singleton-like behavior for serverless
// Note: In serverless, each invocation may create a new instance
