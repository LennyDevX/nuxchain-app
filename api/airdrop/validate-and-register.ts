 ...
 */
async function getCEXWallets(): Promise<Set<string>> {
  const now = Date.now();
  ...
  try {
    ...
    const data = (await response.json()) as { success?: boolean; wallets?: string[] };
...
async function verifyHCaptcha(token: string | undefined): Promise<{ valid: boolean; reason?: string }> {
  ...
  try {
    ...
    console.error('❌ [TURNSTILE] Verification request error (failing open):', (err as Error).message);
    return { valid: true };
  }
}
 ...
 */
async function isFundedByCEX(wallet: string, oldestSignature: string): Promise<boolean> {
  try {
...
async function checkIPFarm(ipAddress: string): Promise<{
  isRisky: boolean;
  count: number;
  reason?: string;
}> {
...
async function checkDuplicates(email: string, wallet: string): Promise<{
  ...
  try {
    ...
    if (!emailSnapshot.empty) {
      await logSecurityViolation(
        ...
        `Duplicate email attempt: ${email} (normalized: ${normalizedEmail})`,
        'unknown',
        email,
        wallet
      );
    ...
    if (!walletSnapshot.empty) {
      await logSecurityViolation(
        EventType.DUPLICATE_DETECTED,
        `Duplicate wallet attempt: ${wallet}`,
        'unknown',
        email,
        wallet
      );
...
export async function validateAirdropRegistration(req: Request, res: Response) {
  const startTime = Date.now();
  try {
    const {
      name,
    ...
    if (isDisposableEmail(email)) {
      await logSecurityViolation(
        EventType.BOT_DETECTED,
        `Disposable email blocked: ${email}`,
        ipAddress,
        email,
        wallet
      );
    ...
    if (isDataCenterIP(ipAddress)) {
      await logSecurityViolation(
        EventType.BOT_DETECTED,
        `Data center IP blocked: ${ipAddress}`,
        ipAddress,
        email,
        wallet
      );
    ...
    if (ipFarmCheck.isRisky) {
      await logSecurityViolation(
        EventType.IP_FARM_DETECTED,
        `IP farm detected: ${ipFarmCheck.count} registrations from ${ipAddress}`,
        ipAddress,
        email,
        wallet,
        { count: ipFarmCheck.count }
      );
      return res.status(429).json({
        success: false,
    ...
    if (fingerprint && fingerprint !== 'unknown') {
      ...
      if (fpCheck.isDuplicate) {
        await logSecurityViolation(
          EventType.DUPLICATE_DETECTED,
          `Duplicate device fingerprint: ${fingerprint}`,
          ipAddress,
          email,
          wallet,
          { fingerprintCount: fpCheck.count }
        );
        return res.status(409).json({
          success: false,
...
export async function submitAirdropRegistration(req: Request, res: Response) {
  try {
    const {
      name,
      email,
    ...
    if (duplicates.hasDuplicate) {
      await logSecurityViolation(
        EventType.DUPLICATE_DETECTED,
        `Duplicate at submission: ${duplicates.field}`,
        ipAddress,
        email,
        wallet
      );
