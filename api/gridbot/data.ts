 ...
 */
async function getBinanceMarkPrice(): Promise<{
  markPrice: number;
  ...
  try {
    ...
    const data = await response.json() as BinanceMarkPrice;
 ...
 */
async function getBinanceKlines(): Promise<PriceHistory[]> {
  const cacheKey = 'binance_klines';
  const cached = getFromCache<PriceHistory[]>(cacheKey, TTL.KLINES);
  
  if (cached) return cached;

  try {
    const response = await fetch(
      `${BINANCE_FUTURES_API}/klines?symbol=BTCUSDT&interval=1h&limit=24`,
      {
        headers: {
          'Accept': 'application/json',
        },
    ...
    const history: PriceHistory[] = data.map((kline) => ({
      timestamp: kline[0],
...
async function getCoinGeckoPrice(): Promise<number | null> {
  ...
  const cached = getFromCache<number>(cacheKey, TTL.COINGECKO);
 ...
 */
async function getCoinGeckoHistory(): Promise<PriceHistory[]> {
  const cacheKey = 'coingecko_history';
  const cached = getFromCache<PriceHistory[]>(cacheKey, TTL.COINGECKO);
  
  if (cached) return cached;

  try {
    const response = await fetch(
      `${COINGECKO_API_BASE}/coins/bitcoin/market_chart?vs_currency=usd&days=1&interval=hourly`,
      {
        headers: {
          'Accept': 'application/json',
          'User-Agent': 'Nuxchain-App/2.0',
    ...
    const history: PriceHistory[] = data.prices.map(([timestamp, price]) => ({
      timestamp,
      price,
    }));
