 ...
 * Verifies a Solana transaction on-chain and registers the purchase in Firestore.
 *
 * Body: { wallet: string, txSignature: string, tier: 1 | 2 }
 *
 * Flow:
 * 1. Validate inputs
 * 2. Verify tx on-chain (check recipient = DEPLOYER_NUX, amount matches tier price × nuxAmount)
 * 3. Check for duplicate txSignature
 * 4. Write to Firestore `launchpadPurchases`
 */
...
const MIN_BUY: Record<number, number> = {
  1: 5_000,
  2: 1_000,
};
...
export default async function handler(req: Request, res: Response) {
  ...
  const { wallet, txSignature, tier } = req.body as {
    wallet: string;
    txSignature: string;
    tier: 1 | 2;
  };
