      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md max-h-[85vh] bg-[#0B0F19]/95 backdrop-blur-3xl border border-[rgba(239,68,68,0.3)] rounded-2xl shadow-2xl overflow-hidden flex flex-col"
            >
              {/* Header */}
              <div className="flex items-center justify-between px-5 py-4 border-b border-[rgba(239,68,68,0.2)]">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h2 className="text-base font-bold text-white">Emergency Tools</h2>
                    <p className="text-xs text-slate-400">Quick protocol actions</p>
                  </div>
                </div>
                <button
                  onClick={onClose}
                  className="w-8 h-8 rounded-lg bg-[rgba(239,68,68,0.1)] hover:bg-[rgba(239,68,68,0.2)] text-[#ef4444] transition-all flex items-center justify-center"
                >
              ...
              <div className="overflow-y-auto px-5 py-4 space-y-4">
                {Object.entries(groupedTools).map(([category, tools]) => (
                  <div key={category}>
                    <h4 className="text-xs font-bold text-[#ef4444] uppercase tracking-wider mb-2 flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className={`p-3 rounded-xl border transition-all text-left ${
                            tool.comingSoon
                              ...
                              : `${BG_COLOR_CLASSES[tool.color as keyof typeof BG_COLOR_CLASSES]} hover:scale-[1.01]`
                          }`}
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between">
                                ...
                                {tool.comingSoon && (
                                  <span className="px-1.5 py-0.5 bg-slate-700/50 text-slate-400 text-[10px] rounded-full font-medium">
                                    Soon
                                  </span>
                                )}
                              </div>
                              <p className={`text-xs mt-0.5 ${tool.comingSoon ? 'text-slate-500' : 'text-slate-400'}`}>
                                {tool.description}
                              </p>
                            </div>
                          </div>
