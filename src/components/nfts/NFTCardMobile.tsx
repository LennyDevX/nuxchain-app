      ...
      >
        ...
        >
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-purple-900/40 via-gray-900 to-black border border-purple-500/30 shadow-2xl h-full w-full flex flex-col">
            ...
            >
              {/* SLIDE 1 - Main Info */}
              <div className="min-w-full h-full snap-start overflow-y-auto custom-scrollbar">
                <div className="p-3 space-y-3">
                  ...
                  {nft.isForSale && nft.price && (
                    <div className="bg-gradient-to-r from-emerald-600/20 to-green-600/20 border border-emerald-500/40 rounded-lg p-3">
                      <div className="flex items-center gap-1.5 mb-2">
                        <svg className="w-4 h-4 text-emerald-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M8.433 7.418c.155-.103.346-.196.567-.267v1.698a2.305 2.305 0 01-.567-.267C8.07 8.34 8 8.114 8 8c0-.114.07-.34.433-.582zM11 12.849v-1.698c.22.071.412.164.567.267.364.243.433.468.433.582 0 .114-.07.34-.433.582a2.305 2.305 0 01-.567.267z" />
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-13a1 1 0 10-2 0v.092a4.535 4.535 0 00-1.676.662C6.602 6.234 6 7.009 6 8c0 .99.602 1.765 1.324 2.246.48.32 1.054.545 1.676.662v1.941c-.391-.127-.68-.317-.843-.504a1 1 0 10-1.51 1.31c.562.649 1.413 1.076 2.353 1.253V15a1 1 0 102 0v-.092a4.535 4.535 0 001.676-.662C13.398 13.766 14 12.991 14 12c0-.99-.602-1.765-1.324-2.246A4.535 4.535 0 0011 9.092V7.151c.391.127.68.317.843.504a1 1 0 101.511-1.31c-.563-.649-1.413-1.076-2.354-1.253V5z" clipRule="evenodd" />
                        </svg>
                        <span className="text-xs text-emerald-300 font-semibold">Current Price</span>
                      </div>
                      <div className="flex items-baseline gap-1">
                        <p className="text-lg font-bold text-white truncate">{formattedPricePOL}</p>
                        {formattedPriceUSD && (
                          <p className="text-xs text-emerald-400 truncate">≈ {formattedPriceUSD}</p>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  {!nft.isForSale && (
                    <button 
                      ...
                      className="w-full backdrop-blur-xl bg-gradient-to-r from-purple-600 to-blue-600 active:from-purple-700 active:to-blue-700 border border-purple-400/50 rounded-xl px-3 py-3 text-sm font-bold text-white transition-all duration-300 active:scale-95 flex items-center justify-center gap-2 touch-manipulation"
                    >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gradient-to-br from-purple-600/20 to-purple-600/5 border border-purple-500/30 rounded-lg p-3">
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <span className="text-[10px] text-purple-300 font-semibold">Creator</span>
                      </div>
                      <p className="text-[10px] text-white font-mono bg-purple-600/20 px-2 py-1.5 rounded truncate">
                    ...
                    <div className="bg-gradient-to-br from-blue-600/20 to-blue-600/5 border border-blue-500/30 rounded-lg p-3">
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <span className="text-[10px] text-blue-300 font-semibold">Owner</span>
                      </div>
                      <p className="text-[10px] text-white font-mono bg-blue-600/20 px-2 py-1.5 rounded truncate">
                  ...
                  {nft.attributes && nft.attributes.length > 0 ? (
                    <>
                      <div className="flex items-center gap-1.5 mb-3">
                        ...
                        <h5 className="text-sm font-bold text-white">Attributes</h5>
                        <span className="ml-auto text-xs text-purple-400 bg-purple-600/20 px-2 py-1 rounded-full flex-shrink-0">
                        ...
                        {nft.attributes.map((attr: NFTAttribute, index: number) => {
                          ...
                          return (
                            <div 
                              ...
                              } border rounded-lg p-2 transition-all duration-200 hover:scale-105 active:scale-95 hover:border-purple-400/50 touch-manipulation`}
                            >
                              <p className="text-[9px] text-gray-400 mb-1 truncate">{attr.trait_type}</p>
                              <p className={`text-xs font-bold text-white truncate ${isSpecialAttr ? 'font-mono text-purple-300' : ''}`}>
                                {formattedValue}
                              </p>
                            </div>
                          );
