      ...
      <div className="p-5 pb-0">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-semibold text-white">Quests & Achievements</h3>
              <p className="text-white/40 text-[10px]">Complete tasks to earn XP and rewards</p>
            </div>
          </div>
          {userXP && (() => {
            ...
            return (
              <div className="flex items-center gap-2">
                <span className={`text-[10px] ${color}`}>{title}</span>
                <div className="px-2.5 py-1 rounded-full text-xs font-bold bg-amber-500/20 border border-amber-500/30 text-amber-300">
        ...
        <div className="bg-white/5 rounded-lg p-3 border border-white/5 mb-4 flex items-center justify-between">
          <div>
            <p className="text-white text-xs font-medium">Auto-Compound</p>
          ...
          {(['quests', 'achievements', 'badges'] as TabType[]).map((tab) => (
            ...
            >
              ...
              {tab === 'achievements' && unclaimedAchievements.length > 0 && (
                <span className="ml-1 px-1.5 py-0.5 rounded-full text-[9px] bg-purple-500/20 text-purple-400">
      ...
      <div className="p-5 pt-3">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'quests' && (
            ...
            >
              {questRewards.length === 0 ? (
                <EmptyState icon="📋" message="No quest rewards available" />
              ) : (
                questRewards.map((quest) => (
                  <div
                    key={quest.questId}
                    className={`bg-white/5 rounded-lg p-3 border flex items-center justify-between ${
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-white text-xs font-medium">Quest #{quest.questId}</span>
                        {quest.isClaimed && (
                      ...
                      <p className="text-white/40 text-[10px]">Reward: {quest.rewardAmount} POL</p>
                    ...
                    {!quest.isClaimed && !quest.isExpired && (
                      <motion.button
                        className="px-3 py-1.5 rounded-lg text-[10px] font-medium bg-amber-500/20 text-amber-400 border border-amber-500/30 hover:bg-amber-500/30 transition-colors"
          ...
          {activeTab === 'achievements' && (
            ...
            >
              {achievementRewards.length === 0 ? (
                <EmptyState icon="🎯" message="No achievements unlocked yet" />
              ) : (
                achievementRewards.map((achievement) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-white/40 text-[10px]">Reward: {achievement.rewardAmount} POL</p>
                    </div>
                    {!achievement.isClaimed && !achievement.isExpired && (
                      <motion.button
                        className="px-3 py-1.5 rounded-lg text-[10px] font-medium bg-purple-500/20 text-purple-400 border border-purple-500/30 hover:bg-purple-500/30 transition-colors"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        disabled={isTransacting}
          ...
          {activeTab === 'badges' && (
            ...
            >
              {badges.length === 0 ? (
                <EmptyState icon="🏅" message="No badges earned yet" />
              ) : (
                <div className="grid grid-cols-3 gap-2">
                  {badges.map((badge) => {
                    ...
                    return (
                      <motion.div
                        key={badge.badgeId}
                        className="bg-white/5 rounded-lg p-3 border border-white/10 text-center group relative"
                        whileHover={{ scale: 1.05, backgroundColor: 'rgba(255,255,255,0.08)' }}
                      >
                        <span className="text-2xl">{meta.icon}</span>
                        <p className="text-white text-[10px] font-medium mt-1 truncate">{meta.name}</p>
                        <p className="text-white/30 text-[9px]">
                          {badge.earnedAt.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                        </p>
                        {/* Tooltip */}
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-10">
                          <div className="bg-gray-900 border border-white/20 rounded-lg px-3 py-2 text-[10px] text-white whitespace-nowrap shadow-xl">
...
const EmptyState: React.FC<{ icon: string; message: string }> = ({ icon, message }) => (
  <div className="bg-white/5 rounded-lg p-6 text-center border border-dashed border-white/10">
    ...
    <p className="text-white/40 text-xs mt-2">{message}</p>
    <p className="text-white/25 text-[10px] mt-1">Keep staking to unlock rewards!</p>
  </div>
);
