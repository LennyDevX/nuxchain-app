...
contract MarketplaceStatistics is AccessControl, IMarketplaceStatistics {
    ...
    // ════════════════════════════════════════════════════════════════════════════════════════
    // STATE VARIABLES - CATEGORY & TIME STATISTICS
    // ════════════════════════════════════════════════════════════════════════════════════════

    mapping(bytes32 => uint256) private _categoryVolume;
    mapping(bytes32 => uint256) private _categorySales;
    mapping(uint256 => uint256) private _dailyVolume;
    ...
    function recordSale(
        ...
        if (bytes(category).length > 0) {
            bytes32 key = keccak256(abi.encodePacked(category));
            _categoryVolume[key] += price;
            _categorySales[key]++;
        }
        _dailyVolume[block.timestamp / 86400] += price;
    ...
    /// @inheritdoc IMarketplaceStatistics
    function getCategoryStats(string calldata category) external view returns (IMarketplaceStatistics.CategoryStats memory) {
        bytes32 key = keccak256(abi.encodePacked(category));
        return CategoryStats({
            volume: _categoryVolume[key],
            sales: _categorySales[key]
        });
    }
    ...
    function getDailyVolume(uint256 daysAgo) external view returns (uint256) {
        ...
        if (daysAgo > today) return 0;
        return _dailyVolume[today - daysAgo];
    }
