...
contract SmartStakingViewSkills {
    ...
    function getUserInfoWithSkills(address user) external view returns (UserInfoWithSkills memory) {
        ...
        result.skillProfile = abi.decode(dataProfile, (IStakingIntegration.UserPowerProfile));
        ...
        if (successSkills) {
            result.activeSkills = abi.decode(dataSkills, (IStakingIntegration.NFTPower[]));
        }
    ...
    function getActiveSkillsWithDetails(address user) external view returns (SkillDetails[] memory) {
        ...
        IStakingIntegration.NFTPower[] memory activeSkills = abi.decode(data, (IStakingIntegration.NFTPower[]));
        SkillDetails[] memory details = new SkillDetails[](activeSkills.length);
        
        for (uint256 i = 0; i < activeSkills.length; i++) {
            details[i].powerType = activeSkills[i].powerType;
    ...
    function getAvailableSkillsConfiguration() external view returns (SkillConfig[] memory) {
        ...
        for (uint8 i = 0; i < 5; i++) {
            ...
            (bool enabled, uint16 effect) = _getSkillConfig(powerType);
    ...
    function getUserDetailedStats(address user) external view returns (UserStats memory) {
        ...
        require(successProfile, "View: getUserSkillProfile call failed");
        IStakingIntegration.UserPowerProfile memory profile = abi.decode(
            dataProfile, 
            (IStakingIntegration.UserPowerProfile)
        );
    ...
    function getSkillEffectiveness(address user) external view returns (SkillEffectiveness[] memory) {
        ...
        for (uint256 i = 0; i < activeSkills.length; i++) {
            ...
            effectiveness[i] = SkillEffectiveness({
                powerType: activeSkills[i].powerType,
    ...
    function _getSkillConfig(IStakingIntegration.PowerType powerType) internal view returns (bool enabled, uint16 effect) {
        // Check if skill is enabled
        (bool successEnabled, bytes memory dataEnabled) = stakingContract.staticcall(
            abi.encodeWithSignature("powerEnabled(uint8)", uint8(powerType))
        );
        if (successEnabled) {
        ...
        (bool successEffect, bytes memory dataEffect) = stakingContract.staticcall(
            abi.encodeWithSignature("powerDefaultEffects(uint8)", uint8(powerType))
        );
        if (successEffect) {
