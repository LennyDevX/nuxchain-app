    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      ...
      >
        ...
        >
          ...
          {/* Password Input — shown once owner wallet is connected */}
          {isConnected && isOwner && (
            <div className="mb-5">
              <label className="block text-xs text-gray-400 mb-2">Access Password</label>
              <div className="relative">
                <input
                  type={showPassword ? 'text' : 'password'}
                  value={password}
                  onChange={e => setPassword(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && !isLoading && password && handleLogin()}
                  placeholder="Enter access password"
                  className="w-full bg-gray-900/60 border border-gray-600/50 text-white placeholder-gray-500 rounded-lg px-4 py-3 pr-10 text-sm focus:outline-none focus:border-green-500/60 transition-colors"
                  autoComplete="off"
                />
                <button
                  type="button"
